...
### Activate an specific environment

At this step, you are in the main environment of julia, but you to create one for your folder and project

```julia
(@v1.5) pkg> activate .
```

With this you have a environment in the folder you run it with the name of the folder. Then install the following in order to have the toml with the packages and things of your environment

```julia
(julia (in this case)) pkg> add ForwardDiff
```
...
This is the main menu - here you can create new notebooks, or open existing ones. To start from a template notebook on the web, you can _paste the URL into the blue box_ and press ENTER.

For example, an example is available [here](https://github.com/victosdur/Julia/blob/main/PlutoNotebookExample.jl). Copy this link (right click -> Copy Link), and paste it into the box. Press ENTER, and select OK in the confirmation box.
